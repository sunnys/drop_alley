defmodule DropAlleyWeb.API.V1.AddressControllerTest do
  ...
  describe "create address" do
    test "renders address when data is valid", %{conn: conn} do
      conn = post conn, api_v1_address_path(conn, :create), address: @create_attrs
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get conn, api_v1_address_path(conn, :show, id)
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "active" => true,
        "addr" => "some addr",
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, api_v1_address_path(conn, :create), address: @invalid_attrs
  ...
  describe "update address" do
    ...
    test "renders address when data is valid", %{conn: conn, address: %Address{id: id} = address} do
      conn = put conn, api_v1_address_path(conn, :update, address), address: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, address: address} do
      conn = put conn, api_v1_address_path(conn, :update, address), address: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete address" do
    ...
    test "deletes chosen address", %{conn: conn, address: address} do
      ...
      assert_error_sent 404, fn ->
        get conn, api_v1_address_path(conn, :show, address)
      end
    end
  end
